...
import { showFilterBarAction } from "../../../Redux/fetchProduct/fetchProductSlice";
...
const MainFilterBar = () => {
  ...
  const { filterBar } = useSelector(state => state.products);

  return (
    <>
      <Overlay filterBar={filterBar} onClick={() => dispatch(showFilterBarAction())} />
      <Wrapper filterBar={filterBar} onClick={e => e.stopPropagation()}>
        <span className="closeIcon" onClick={() => dispatch(showFilterBarAction())}>
          <IoMdClose />
        </span>
        <Section>
          <h4>Categories</h4>
